...
func3 = kappa_d_e * WW(rho) * dx
func4 = kappa_m_e * inner(grad(v_g(rho)), grad(v_g(rho))) * dx
PP = func3 + func4
...
L_heat_adjoint = w * h_r(rho) * inner(sigma_A(Id, Id), epsilon(p)) * dx
...
L_lagrange = inner(f, p) * ds(8) + s * h_r(rho) * inner(sigma_A(Id, Id), epsilon(p)) * dx
